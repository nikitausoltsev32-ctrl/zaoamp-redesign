...
export function BenefitCard({ benefit, index, className = '' }: BenefitCardProps) {
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    // Synchronously store the latest position to avoid stale data
    lastPos.current = { x: e.clientX - rect.left, y: e.clientY - rect.top }

    if (!ticking.current) {
      // ⚡ Bolt: Throttle frequent mousemove events using requestAnimationFrame to prevent unnecessary re-renders
